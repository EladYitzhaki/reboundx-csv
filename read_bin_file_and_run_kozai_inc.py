...
#header.append("inc1+inc2")
header.append("inc_tot")
...
for i in range(10000):
    ...
    with open(filename1, 'a') as file:
        ...
        p1 = sim.particles[1]
        p2 = sim.particles[2]
        J1 = p1.m * np.cross([p1.x, p1.y, p1.z], [p1.vx, p1.vy, p1.vz])
        J2 = p2.m * np.cross([p2.x, p2.y, p2.z], [p2.vx, p2.vy, p2.vz])
        inc_tot = np.arccos(np.inner(J1, J2) / (np.sqrt(np.inner(J1, J1)) * np.sqrt(np.inner(J2, J2))))
    ...
    #list_at_time.append(orbs[0].inc+orbs[1].inc)
    list_at_time.append(inc_tot*180/np.pi)
